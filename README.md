...
### ✅ Authentication APIs (Signup / Login) using bcrypt + JWT

This project implements a secure authentication system using **bcrypt** for password hashing and **JWT (JSON Web Tokens)** for session management. The Signup API ensures that user passwords are never stored in plain text by hashing them before saving to the database, protecting user credentials even in case of a database leak. The Login API verifies credentials by comparing the entered password with the stored hashed password, and upon successful authentication, generates a signed JWT token containing the user’s identity. This token is then used to access protected routes, where the backend verifies the JWT to ensure only authenticated users can access private data. Token expiration is also enforced to enhance security, making the authentication flow reliable, scalable, and production-ready for real-world applications.

### ✅ Authorization Middleware (RBAC with JWT Validation)

This project implements a reusable **Authorization Middleware** to enforce secure access control across protected API routes using **JWT verification** and **Role-Based Access Control (RBAC)**. While authentication ensures the user is valid, this middleware ensures the user is only allowed to access routes based on their assigned role (e.g., `admin` vs `user`). The middleware intercepts incoming requests, validates the JWT token, checks session validity, and restricts access to sensitive endpoints such as `/api/admin` using the principle of **least privilege**. If the token is missing, expired, or the role does not match the required permission level, the API returns a clear denied response. This centralized approach makes authorization consistent across the application and allows easy scalability for future roles like `moderator` or `editor`.
